    ...
    for (self.pattern) |layer, py| {
        for (layer) |row, pz| {
            for (row) |block_opt, px| {
                ...
                if (block_opt) |block| {
                    // normales Matching
                    if (block != block_at) return false;
                } else {
                    // null = alles außer Bedrock
                    if (block_at == Block.bedrock) return false;
                }
            }
        }
    }
...
pub const GradientGenerator = struct {
    ...
    pub fn floor(seed: i64, lower_y1: i32, upper_y1: i32) GradientGenerator {
